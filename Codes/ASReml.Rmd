...
\[
  ...
  X= \text{is a m x n design matrix relating phenotypes to fixed effects of replications and environments}\\
  \beta = \text{ is a vector of fixed effects}\\
  u= \text{is a random effect with variance u}\\
  \epsilon= \text{is  n x n matrix of residual/error effects }\\
\]
here, we assume var(u)~$\sigma^2_{g}G$, where G is genomic relationship  kinship co-variance matrix of n x m dimensions (n is no. of markers and m is no. of individuals) representing genomic similarity of individuals, and  $Var(\epsilon)= \sigma^2_{\epsilon}I$, I is identity matrix.
