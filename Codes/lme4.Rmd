...
```{r}
# 
model2<- lmer(Yield~Environment+(1|Genotype)+
...
***
\[
Y_{ijk}= \mu + G_{i} + E_{j} + G_{i}*E_{j}+ R_{k}(E_{j})+ B_{l}(R_{K}(E_{j}))+ \varepsilon_{ijk}\\
...
R_{k}(E_{j})= \text {is effect of $k$th replication nested in $j$th environment}\\
B_{l}(R_{K}(E_{j}))=  \text {effect of $k$th block nested within $k$th and$j$th environment}\\
e_{ijk}=\text{error}\\
...
```{r}
demo.data.filtered$Environment<-as.factor(demo.data.filtered$Environment)
Model3.lme4<-lmer(Yield~Environment+(1|Genotype)+(1|Environment:Genotype)+
