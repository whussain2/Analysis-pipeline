...
***
\[
Y_{ij}= \mu+G_{i} + B_{j} + \epsilon_{ij}\\
...
\varepsilon_{ij}=\text{residual error}\\
...
***
\[
Y_{ijk}= \mu+G_{i} + B_{j}+ C_{k} + \epsilon_{ijk}\\
...
\[
Y_{ijk}= G_{i} + B_{j}+ C_{k} + \epsilon_{ijk}\\
Y_{ijk}= \text{ is the effect of $i$th genotype in $j$th block and $k$th column} \\
\mu= \text {overall mean}\\
G_{i}=\text{random effect of the $i$th genotype}\\
B_{j}= \text {random efect of $j$th block}\\
...
\[
Y_{ijk}= \mu+G_{i} + E_{j}+G_{i}*E_{j}+ B_{k}(E_{j}) + \epsilon_{ijk}\\
