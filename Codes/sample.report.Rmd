...
libraries("dplyr", "reshape2", "readxl", "ggpubr","stringr", "ggplot2", 
          ...
          "ggcorrplot", "RColorBrewer", "hrbrthemes", "tm", "proustr")
```
...
***
* Demo data set used is in augmented RCBD design. 
...
# View as table
  ...
  print_table(demo.data[, c(1, 5,6,8,9,13)], editable = 'cell', rownames = FALSE, caption = htmltools::tags$caption("Table: Raw Data in normal and stress trial",style="color:black; font-size:130%"), filter = 'top')
```
...
* Here in this step data will be pre-processed and quality of data will be checked, and only quality phenotypes will be advanced for downstream analysis and get more reliable and accurate estimates or predictors. 

* The steps in pre-processing involves:
  - Looking for missing data
  - Descriptive statistics for each variable
  - Heatmaps of field experimental design
  - Data visualizatiosn as boxplots, histograms and QQ plots
...
* Here data is summarized as mean, median, standard deviation (SD), minimum, maximum, coefficient of variation (CV) and standard error (SE) for grain yield, days to flowering, and plant height traits.
...
<button class="btn btn-primary" data-toggle="collapse" data-target="#HeatMap"> Click to show Heatmaps/Hide </button>  
<div id="HeatMap" class="collapse">  

**Showing heat maps of field design under non-stress and drought environments. X axis shows the list of columns and y-axis the blocks (blocks here are also treated as rows).**
...
<div class = "blue">
* As the data is in **Augmenetd RCBD** with one replication, uni-variate approach will be used to find the potential outliers.
...
* Demo data analysis for grain yield will also be shown using freely available [lme4 R Package](https://cran.r-project.org/web/packages/lme4/lme4.pdf) package, and will be useful to the users which do not have access to the commercial **ASReml-R package**.
...
<span style="color: darkred;font-size: 25pt">**Combined Data Summary and Heritabilty**</span>
...
```{r, fig.height=6, fig.width=10,  fig.cap="Bar plot showing the BLUPs for top 10% of genotypes and all the checks. Dased lines shows overall mean of all genotypes and checks. Genotypes differ slightly from checks and mean of entries and checks are almost close."}
...
***
# Phenotypic Data Analysis in lme4
***
...
* The purpose of this section is repeat the phenotypic data anlysis in lme4 as ASReml R package is commercial package and may not available to all the users. 

* We will use filtered data set to show and perform the analysis in lme4.
...
* We will use the following model in lme4 to run the analysis
\[
Y_{ij}= \mu+G_{i} + B_{j} + \varepsilon_{ij}\\
Y_{ij}= \text{ is the effect of $i$th genotype in $j$th block} \\
\mu= \text {overall mean}\\
G_{i}=\text{effect of the $i$th genotype}\\
B_{k}= \text {effect of $k$th block}\\
e_{ij}=\text{error}\\
\text{here we assume residuals are independent and identically distributed }\varepsilon\sim \text{$iid$N}(0,\sigma_e^2)\\
\]

* The model described above is equivalent to model1 described and used in ASReml R package.

* Note: Blocks here are fixed and Genotypes as Random
...
```{r}
# Extract the varaince components
...
### First Get Basic ANOVA

```{r}
demo.data.out$Environment<-as.factor(demo.data.out$Environment)
str(demo.data.out)
model.anova<-lm(formula = GYKGPHA~Genotype+Block+Environment,data=demo.data.out)

# Get ANOVA
anova(model.anova)
```

### Check for Homogeneity of Varaince
...
\[
Y_{ij}= \mu+G_{i} + B_{j} + e{_k}+ \varepsilon_{ij}\\
Y_{ij}= \text{ is the effect of $i$th genotype in $j$th block and $k$th environment} \\
\mu= \text {overall mean}\\
G_{i}=\text{effect of the $i$th genotype}\\
B_{j}= \text {effect of $j$th block}\\
E{_k}= \text{ is the effect of $k$th environment}\\
e_{ij}=\text{error}\\
\text{here we assume residuals are independent and identically distributed }\varepsilon\sim \text{$iid$N}(0,\sigma_e^2)\\
\]
...
* We will extract varaince components and also calculate heritability.

```{r}
str(demo.data.out)
demo.data.out$Environment<-as.factor(demo.data.out$Environment)
model3<-lmer(GYKGPHA ~Block+(1|Genotype)+(1|Environment), data=demo.data.out1)
summary(model3)
plot(model3)
# Extract the varaince components
...
# Now calculate heritability using variance components
genotype.var=Ve[1,4]
environment.var=Ve[2,4]
error.var=Ve[3,4]
# Now heritability
h2=genotype.var/(genotype.var+environment.var+error.var)*100
h2
# Relaibility
library(arm)
...
***
*Note: For questions specific to analysis please contact waseem.hussain@irri.org*
